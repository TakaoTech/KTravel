...
`MaterialTheme` is the root provider for design tokens in Compose Material 3. It establishes `colorScheme`,
`typography`, and `shapes` across your app.
...
This requires `android:READ_MEDIA_IMAGES` or context access. `dynamicColorScheme` APIs are in
