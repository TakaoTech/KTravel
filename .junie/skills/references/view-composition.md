...
**Key principle:** Slots accept `@Composable` lambdas, not pre-composed values. This ensures composition is deferred and
scope-aware.
...
**Advantage:** Caller can test and reuse `ToggleButton` without mocking state. `StatefulToggleButton` provides
...
**Rationale:** Composables are executed during composition, which happens at unpredictable times and may be skipped or
reordered.
...
**Source references:** `androidx.compose.material3`, `androidx.compose.ui.tooling.preview`,
