...
@Stable
data class TransportPlanningUiState(
    ...
    /**
     * That a calculation has just succeeded and the preview is what should be on screen.
     *
     * A one-shot the host clears through [TransportPlanningViewModel.onPreviewOpened]: the answer
     * itself lives in [RouteAnswerDraft], and this only says that it is new.
     */
     ...
     *
     * True while no profile is chosen yet, rather than false: the option is only ever *withdrawn*
     * by a profile that says it cannot honour it, and a catalog that has not answered yet has said
     * no such thing. Nothing can be computed in that state anyway — [canCalculate] sees to that —
     * and a choice made before the profile arrives is dropped by
     * [TransportPlanningViewModel.selectProfile] if that profile turns out to refuse it.
     */
