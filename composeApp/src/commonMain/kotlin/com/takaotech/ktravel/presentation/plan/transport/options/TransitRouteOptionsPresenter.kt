 ...
 *
 * Same contract as the routing family — it owns the request in the plan's draft and nothing above
 * it knows what a transfer is — with one difference that follows from the API rather than from
 * taste: there is no chosen vehicle, so there is no per-vehicle block to nest.
 */
