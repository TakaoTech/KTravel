...
/**
 * The routing family: which vehicle, and how many routes.
 *
 * Owns what does not depend on the vehicle. Everything that does — what to keep off the route, and
 * whether to optimize for distance — belongs to [RoutingModeExtrasScreen], which this presenter
 * picks out of the rules and hands to the UI to render inside itself.
 *
 * The request lives in the plan's
 * [com.takaotech.ktravel.presentation.plan.transport.RouteOptionsDraft] rather than in this
 * composition, because the screen leaves the composition when a route is computed and the traveller
 * expects to find their choices on the way back.
 */
@CircuitInject(RoutingRouteOptionsScreen::class, AppScope::class)
@Composable
fun RoutingRouteOptionsPresenter(
    screen: RoutingRouteOptionsScreen,
    planningGraphStore: PlanningGraphStore,
): RoutingRouteOptionsUiState {
