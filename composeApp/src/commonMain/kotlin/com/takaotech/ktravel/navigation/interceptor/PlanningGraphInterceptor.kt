 ...
 */
internal class PlanningGraphInterceptor(private val store: PlanningGraphStore) : NavigationInterceptor {

    override fun goTo(screen: Screen, navigationContext: NavigationContext): InterceptedResult {
        if (screen is PlanOverviewScreen) store.getOrCreate(screen.travelId)
    ...
    override fun pop(result: PopResult?, navigationContext: NavigationContext): InterceptedResult {
        ...
        if (leaving is PlanOverviewScreen) store.release(leaving.travelId)
        return NavigationInterceptor.Skipped
    }
}
