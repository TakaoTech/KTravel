...
internal const val MAX_SCORE_STEPS = 5f

/**
 * Shows the verdict of `:password-strength` on a password being typed.
 *
 * The module returns symbolic warnings and suggestions rather than text, so all of the wording
 * lives here in `strings.xml` — which is what keeps the module free of anything to translate.
 */
@Composable
...
@Composable
internal fun Int.label(): String = stringResource(
    when (this) {
...
@Composable
internal fun Int.toColor(): Color = when (this) {
...
@Composable
internal fun PasswordWarning.message(): String = stringResource(
    when (this) {
...
@Composable
internal fun PasswordSuggestion.message(): String = stringResource(
    when (this) {
