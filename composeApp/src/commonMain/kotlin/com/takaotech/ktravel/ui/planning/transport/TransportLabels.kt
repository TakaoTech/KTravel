 ...
 * profile has. `BUS` reads the same in both because the Italian for a coach and for a service bus is
 * the same word — but which of the two the traveller is choosing is still settled by the profile the
 * chip belongs to, never by this table.
 */
 ...
 *
 * Null is a normal answer and not a gap to work around: the chip carries its label either way, and a
 * mode without an icon reads as a word rather than as a wrong picture.
 */
