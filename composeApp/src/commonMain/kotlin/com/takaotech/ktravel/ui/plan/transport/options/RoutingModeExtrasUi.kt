...
import com.takaotech.ktravel.presentation.plan.transport.options.RoutingModeExtrasEvent
import com.takaotech.ktravel.presentation.plan.transport.options.RoutingModeExtrasScreen
import com.takaotech.ktravel.presentation.plan.transport.options.RoutingModeExtrasUiState
...
internal fun RoutingModeExtrasContent(
    ...
    Column(modifier = modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(22.dp)) {
        ...
        ToggleRow(
            modifier = Modifier.testTag(TransportPlanningTestTags.SHORTEST),
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
internal fun FilterChipFor(feature: RouteFeature, selected: Boolean, onClick: () -> Unit) {
    FilterChip(
        modifier = Modifier.testTag(TransportPlanningTestTags.avoidTag(feature.name)),
        selected = selected,
        onClick = onClick,
