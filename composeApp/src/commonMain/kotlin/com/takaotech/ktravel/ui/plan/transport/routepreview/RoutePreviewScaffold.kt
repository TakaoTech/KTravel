 ...
 *
 * On a wide window the list and the map sit side by side; on a narrow one the list is a bottom
 * sheet over the map. The map is told when the sheet covers it so its gestures stop competing with
 * the drag that is pulling the sheet.
 *
 * @param alternatives How many options the traveller can choose between. A single one draws no
 *   tabs.
 */
