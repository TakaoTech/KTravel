 ...
 *
 * FileKit does not expose the MIME type of a picked file, so [fromExtension] derives it from the
 * file extension; the fallback is [DEFAULT].
 */
internal object MimeTypes {

    /** Shared shape of every catalog entry: the extensions it covers and the media type they map to. */
    interface Entry {
        /** Extensions covered by this entry, lowercase and dot prefixed. */
    ...
    enum class Application(override val extensions: Array<String>, override val mimeType: String) : Entry {
        /** AbiWord document */
    ...
    enum class Audio(override val extensions: Array<String>, override val mimeType: String) : Entry {
        /** AAC audio */
    ...
    enum class Image(override val extensions: Array<String>, override val mimeType: String) : Entry {
    ...
    enum class Text(override val extensions: Array<String>, override val mimeType: String) : Entry {
        ...
        /** Markdown (RFC 7763) */
        MARKDOWN(arrayOf(".md", ".markdown"), "text/markdown"),
    ...
    enum class Video(override val extensions: Array<String>, override val mimeType: String) : Entry {
    ...
    enum class Font(override val extensions: Array<String>, override val mimeType: String) : Entry {
        /** OpenType font */
    ...
    /** Media type used when the extension is unknown. */
    val DEFAULT: MimeType = MimeType(Application.OCTET_STREAM.mimeType)

    // A few extensions appear in more than one enum (".mid" in both MIDI entries): the first
    // group that claims one wins, so the order below is the lookup precedence.
    private val byExtension: Map<String, MimeType> = buildMap {
        val groups = listOf(
            Image.entries,
            Video.entries,
            Audio.entries,
        ...
        groups.forEach { group ->
            group.forEach { entry ->
                entry.extensions.forEach { extension ->
                    getOrPut(extension.lowercase()) { MimeType(entry.mimeType) }
                }
            }
        }
    }
    ...
    /**
     * Media type for [extension], with or without the leading dot and in any case.
     * Falls back to [DEFAULT] when the extension is unknown or blank.
     */
    fun fromExtension(extension: String): MimeType {
        val normalized = extension.lowercase().let { if (it.startsWith('.')) it else ".$it" }
        return byExtension[normalized] ?: DEFAULT
    }
}
