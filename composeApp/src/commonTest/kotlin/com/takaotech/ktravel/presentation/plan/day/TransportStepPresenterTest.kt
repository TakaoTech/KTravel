...
class TransportStepPresenterTest : BehaviorSpec() {

    private val screen = TransportStepScreen(TRAVEL_ID, DAY_ID, TRANSPORT_ID)
    ...
    private suspend fun com.slack.circuit.test.CircuitReceiveTurbine<TransportStepUiState>.awaitLoaded():
        ...
        given("a transport sitting between two places") {
            ...
            `when`("a Recalculate event is sent") {
                then("it should open the computation flow on those two places") {
                    ...
                    presenterTestOf({
                        TransportStepPresenter(screen, navigator, createStore(repository), attachments())
                    }) {
                        awaitLoaded().eventSink(TransportStepEvent.Recalculate)
            ...
            `when`("a note event is sent") {
                then("it should reach the repository through the shared notes block") {
                    ...
                    presenterTestOf({
                        ...
                        state.eventSink(
                            TransportStepEvent.Notes(StepNotesEvent.NoteChanged("# Stop at the services")),
                        )
