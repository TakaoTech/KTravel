 ...
 */
class StepAttachmentTest : BehaviorSpec({
    ...
    fun planWithPlaceStep(attachments: List<AttachmentDomain> = emptyList()): TravelPlanDomain {
        val step = StepDomain.Place(
            id = "s1", name = "Colosseo", lat = 41.89, lng = 12.49, attachments = attachments
        )
        val day =
    ...
    given("a Step.Place with an attachment") {
        val step = StepDomain.Place(
            id = "s1", name = "Trevi", lat = 1.0, lng = 2.0, attachments = listOf(attachment)
        )
    ...
    given("a legacy place entity json without the attachments field") {
        ...
        val legacy = """{"type":"place","id":"s1","location":"Colosseo","lat":41.89,"lng":12.49}"""
