...
class PlanningDetailLayoutTest : BehaviorSpec() {

    init {
        given("PlanningDetailLayout on a compact window") {
            `when`("the backlog is open") {
                then("the scrim should cover the itinerary") {
                    runComposeUiTest {
                        setContent { Layout(width = 390, backlogOpen = true) }
                        onNodeWithTag(PlanningDetailTestTags.BACKLOG_PANE).assertIsDisplayed()
                        onNodeWithTag(PlanningDetailTestTags.SCRIM).assertIsDisplayed()
                    }
                }
                ...
                then("clicking the scrim should dismiss the backlog") {
                    ...
                    runComposeUiTest {
                        ...
                        onNodeWithTag(PlanningDetailTestTags.SCRIM).performClick()
                    }
            ...
            `when`("the backlog is closed") {
                then("neither the pane nor the scrim should exist") {
                    runComposeUiTest {
                        ...
                        onNodeWithTag(PlanningDetailTestTags.BACKLOG_PANE).assertDoesNotExist()
                        onNodeWithTag(PlanningDetailTestTags.SCRIM).assertDoesNotExist()
        ...
        given("PlanningDetailLayout on an expanded window") {
            `when`("the backlog is open") {
