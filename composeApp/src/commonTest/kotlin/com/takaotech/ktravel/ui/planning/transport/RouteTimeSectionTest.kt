        ...
        given("a leg that leaves now") {
            `when`("the traveller asks to leave at an hour instead") {
                then("the mode is reported and nothing is chosen on the screen's behalf") {
                    runComposeUiTest {
                        ...
                        onNodeWithTag(PlanningTransportTestTags.TIME_NOW).assertIsSelected()
                        onNodeWithTag(PlanningTransportTestTags.TIME_DEPART_AT).performClick()
            ...
            then("no hour is shown, because leaving now has none") {
                runComposeUiTest {
                    ...
                    onNodeWithTag(PlanningTransportTestTags.TIME_VALUE).assertDoesNotExist()
                }
            }
        }
        ...
        given("a leg with a departure fixed at an hour") {
            then("that hour is shown on the day the leg belongs to") {
                runComposeUiTest {
                    ...
                    onNodeWithTag(PlanningTransportTestTags.TIME_DEPART_AT).assertIsSelected()
                    onNodeWithTag(PlanningTransportTestTags.TIME_VALUE).assertIsDisplayed()
        ...
        given("a leg with an arrival to be on time for") {
            then("the arrival segment is the selected one") {
                runComposeUiTest {
                    ...
                    onNodeWithTag(PlanningTransportTestTags.TIME_ARRIVE_BY).assertIsSelected()
                }
            }
        }
        ...
        given("a profile that cannot plan backwards from an arrival") {
            then("the arrival segment is not offered, while the other two still are") {
                runComposeUiTest {
                    ...
                    onNodeWithTag(PlanningTransportTestTags.TIME_ARRIVE_BY).assertIsNotEnabled()
                    onNodeWithTag(PlanningTransportTestTags.TIME_NOW).assertIsEnabled()
                    onNodeWithTag(PlanningTransportTestTags.TIME_DEPART_AT).assertIsEnabled()
                }
            }
        }
        ...
        given("a traveller wondering what now is measured from") {
            then("the explanation stays behind the info icon until it is asked for") {
                runComposeUiTest {
                    ...
                    onNodeWithTag(PlanningTransportTestTags.TIME_NOW_INFO, useUnmergedTree = true)
                        .assertIsDisplayed()
                    onNodeWithTag(PlanningTransportTestTags.TIME_NOW_TOOLTIP).assertDoesNotExist()
                }
            }
            ...
            `when`("the info icon is long pressed") {
                then("the explanation is shown") {
                    runComposeUiTest {
                        ...
                        onNodeWithTag(PlanningTransportTestTags.TIME_NOW_TOOLTIP).assertIsDisplayed()
                    }
                }
            }
