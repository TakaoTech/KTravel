 ...
 */
class ZipArchiveTest : BehaviorSpec({
    ...
    given("a zip archive factory") {
        ...
        `when`("entries are written and the archive is reopened") {
            ...
            factory.writer(archive.toKotlinxIoPath()).use { writer ->
                writer.writeEntry("manifest.json", """{"schema_version":1}""".encodeToByteArray())
        ...
        `when`("a missing entry is requested") {
            ...
            factory.writer(archive.toKotlinxIoPath()).use { it.writeEntry("a.txt", byteArrayOf(1)) }
