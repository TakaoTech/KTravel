...
class TravelArchiveRoundTripTest : BehaviorSpec({
    ...
    suspend fun Installation.seedPlanWithFiles(plan: TravelPlanEntity) {
        ...
        attachments.resolveFile(ArchiveTestFixtures.DOC_PATH).write(ArchiveTestFixtures.DOC_BYTES)
    }
    ...
    given("a travel plan with attachments exported from one installation") {
        ...
        `when`("the archive is imported into the other installation") {
            ...
            then("the transport step survives with its route") {
                val transport = imported.days.first().steps.filterIsInstance<StepEntity.Transport>()
            ...
            then("no note reference is dangling") {
                val step = imported.days.first().steps.filterIsInstance<StepEntity.Place>().first()
