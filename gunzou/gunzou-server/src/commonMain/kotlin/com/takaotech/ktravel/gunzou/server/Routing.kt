    ...
    routing {
        ...
        rateLimit(NAVIGATOR_ROUTING_LIMIT) {
            post(NavigatorApi.HERE_ROUTING_TEMPLATE) {
                ...
                val road = HereRoutingCall(
                    transportMode(),
                    call.receive<HereRoutingRequest>(),
                )
 ...
 *
 * @throws com.takaotech.ktravel.gunzou.server.endpoint.NavigatorException [ErrorCode.INVALID_REQUEST] when the segment is not a mode this
 *   contract knows.
 */
private fun RoutingContext.transportMode(): HereTransportMode {
    ...
    return HereTransportMode.fromPathSegment(segment)
 ...
 * leave as [com.takaotech.ktravel.gunzou.server.endpoint.NavigatorException] and are rendered by [configureStatusPages].
 *
...
private suspend inline fun <REQ : Any, reified RES : Any> RoutingContext.respondWithRoute(
    endpoint: com.takaotech.ktravel.gunzou.server.endpoint.NavigationEndpoint<REQ, RES>,
    request: REQ,
