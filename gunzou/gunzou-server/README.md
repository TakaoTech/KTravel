...
The deployable artifact lives in `:gunzou-server-app`, a thin JVM module: the Ktor Gradle plugin
...
This replaces `gunzou-server-app/src/main/resources/openapi/documentation.yaml`, which had
...
| Target | Entry point |
|---|---|
| JVM | `:gunzou-server-app`, `EngineMain` + `application.conf` → `jvmModule()` |
...
`:gunzou-server-app` keeps the fixed `8080` / `$PORT` instead: it is a deployable artifact, and a
...
| Task | Description |
|---|---|
| `./gradlew :gunzou-server:assemble` | Builds all three targets, including the iOS frameworks |
| `./gradlew :gunzou-server:jvmTest` | Runs `commonTest` on the JVM |
| `./gradlew :gunzou-server:testAndroidHostTest` | Runs `commonTest` on the Android JVM |
| `./gradlew :gunzou-server:iosSimulatorArm64Test` | Runs `commonTest` on the iOS simulator |
| `./gradlew :gunzou-server-app:run` | Runs the server on `http://0.0.0.0:8080` |
| `./gradlew :gunzou-server-app:buildFatJar` | Builds the deployable fat JAR |
